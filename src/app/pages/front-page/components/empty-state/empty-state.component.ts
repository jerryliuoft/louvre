...
@Component({
  ...
  template: `
    <div
      class="flex-column-container align-items-center justify-content-center"
      style="height: 100%; min-height: 50vh; width: 100%; text-align: center;"
    >
      <div class="flex-column-container align-items-center" style="margin-bottom: 3rem;">
        ...
        <div class="flex-container align-items-center" style="color: #888; margin-top: 1rem;">
          Click the <mat-icon style="margin: 0 8px;">folder</mat-icon> icon in the header to pick a
          folder and get started!
        </div>
      </div>

      @if (filesService.recentDirectories().length > 0) {
        <div
          class="flex-column-container align-items-center"
        ...
        >
          ...
          <div class="flex-column-container" style="width: 100%; gap: 12px;">
            @for (dir of filesService.recentDirectories(); track dir.name) {
              <button
                mat-stroked-button
                (click)="resumeFolder(dir)"
                class="recent-folder-btn"
                style="width: 100%; height: 50px; display: flex; justify-content: flex-start; align-items: center; padding: 0 16px;"
              >
  ...
  styles: [
    `
      ...
      .recent-folder-btn ::ng-deep .mat-mdc-button-touch-target {
        display: none;
      }
    `,
  ],
